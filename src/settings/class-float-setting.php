...
/**
 * Class representing a float setting.
 *
 * @since 1.0.0
 */
class Float_Setting extends Abstract_Setting {
	...
	protected function default_validation_callback( WP_Error $validity, $value ) {
		...
		if ( isset( $this->schema[ self::ARG_MINIMUM ] )
			&& $value < (float) $this->schema[ self::ARG_MINIMUM ] ) {
			$validity->add( 'value_too_small', sprintf( __( 'The value must not be smaller than %s.', 'wp-gdpr-cookie-notice' ), number_format_i18n( (float) $this->schema[ self::ARG_MINIMUM ] ) ) );
		}

		if ( isset( $this->schema[ self::ARG_MAXIMUM ] )
			&& $value > (float) $this->schema[ self::ARG_MAXIMUM ] ) {
	...
	protected function prepare_schema( array $schema ) : array {
		if ( empty( $schema[ self::ARG_TYPE ] ) ) {
			$schema[ self::ARG_TYPE ] = 'number';
		}

		if ( ! isset( $schema[ self::ARG_DEFAULT ] ) ) {
			$schema[ self::ARG_DEFAULT ] = 0.0;
		}
